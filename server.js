...
【구도·포즈 다양화 규칙 — 필수】
- 이전 2개 씬과 반드시 다른 샷 타입·카메라 앵글을 사용하라
- 금지: 같은 카메라 위치·샷 타입 연속 2회 이상 (예: 정면 medium shot 두 번 연속 금지)
- 금지: same standing pose 연속 2회 이상 — 매 씬마다 인물 동작이 바뀌어야 함
- 아래 카메라 유형을 순환해서 사용하라: close-up → medium shot → over-the-shoulder → wide shot → side profile → low angle → top-down → silhouette → tracking composition
...
PROMPT STRUCTURE (comma-separated, natural English, in this exact order):
1. Character & action: ${charNote ? 'MUST match: ' + charNote + ' — ' : ''}who is doing what RIGHT NOW (most important — be specific, not generic)
2. Core scene & emotion: what is happening and the feeling/atmosphere
3. Location & time of day (specific, from EN_PROMPT)
4. Camera shot — use a DIFFERENT type from the 2 previous scenes, rotating through: close-up, medium shot, over-shoulder, wide shot, side profile, low angle, top-down, silhouette, tracking
5. Ambient motion (wind, particles, fabric, water, fire — pick what fits the style)
6. "5 seconds", "smooth motion", "4K quality"

⚠️ DO NOT write the image style name in the prompt body — it is already locked via STYLE LOCK above.
⚠️ DO NOT truncate or abbreviate the character description — include it fully even if longer.

Rules:
- NO text, subtitles, watermarks
- EN prompt: max 550 characters — if over, cut style decoratives first, NEVER cut character or action
- KO prompt: max 220 characters (Korean, same meaning)
- Each scene MUST use a distinct camera angle and subject pose from the 2 previous scenes
- Same standing pose two scenes in a row: FORBIDDEN
...
// ── 영상 프롬프트 후처리: style dedupe · comma 정리 · 길이 제한 ─────────────
// charDesc·action은 절대 보호. style 장식어를 우선 제거.
function postProcessVideoPrompt(prompt, imageStyle, charDesc, maxLen) {
  if (!prompt || !prompt.trim()) return prompt;
  let p = prompt.trim();

  // [1] imageStyle 중복 제거 — 첫 등장 이후 동일 문구 모두 제거
  if (imageStyle) {
    const esc = imageStyle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const firstIdx = p.toLowerCase().indexOf(imageStyle.toLowerCase());
    if (firstIdx !== -1) {
      const before = p.slice(0, firstIdx + imageStyle.length);
      const after  = p.slice(firstIdx + imageStyle.length)
        .replace(new RegExp('[,\\s]*' + esc, 'gi'), '');
      p = before + after;
    }
  }

  // [2] 반복 style adjective 제거 (같은 단어가 두 번 나오면 두 번째 삭제)
  ['watercolor', 'pastel', 'anime', 'ghibli', 'cinematic', 'painterly',
   'dreamy', 'illustration', 'realistic', 'brush stroke'].forEach(w => {
    const re = new RegExp(`(\\b${w}\\b[^,]{0,80},?\\s*)(\\b${w}\\b)`, 'gi');
    p = p.replace(re, '$1');
  });
  ...
  p = p
    .replace(/,\s*,+/g, ',')     // ,, 제거
    .replace(/,\s*$/g, '')        // 맨 뒤 comma
    .replace(/^\s*,+\s*/g, '')    // 맨 앞 comma
    .replace(/\s{2,}/g, ' ')      // 이중 공백
    .trim();

  // [4] 길이 초과 시 trim — charDesc·action 보호
  if (maxLen && p.length > maxLen) {
    // charDesc 끝 위치 추정 (첫 15자로 위치 탐색)
    let charEnd = -1;
    if (charDesc && charDesc.length > 5) {
      const probe = charDesc.slice(0, 15).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const m = p.match(new RegExp(probe, 'i'));
      if (m) charEnd = p.toLowerCase().indexOf(m[0].toLowerCase()) + charDesc.length;
    }
    // 보호 구간 = charEnd 또는 max(maxLen의 40%, 전체 길이의 40%)
    const safeMin = Math.max(charEnd, Math.floor(maxLen * 0.4));

    // maxLen 이전 마지막 ", " 위치에서 자름 (safeMin 이상이어야 함)
    let cut = p.lastIndexOf(', ', maxLen);
    if (cut < safeMin || cut === -1) cut = maxLen;
    p = p.slice(0, cut).replace(/,\s*$/, '').trim();
  }
...
PROMPT STRUCTURE (comma-separated, natural English, in this exact order):
1. Character & action: ${charNote ? 'MUST match: ' + charNote + ' — ' : ''}who is doing what RIGHT NOW (specific, not generic)
2. Core scene & emotion: what is happening and the emotional atmosphere
3. Location & time of day (from IMAGE_CONTEXT — specific)
4. Camera shot — use a DIFFERENT type from the 2 previous scenes, rotating through: close-up, medium shot, over-shoulder, wide shot, side profile, low angle, top-down, silhouette, tracking
5. Ambient motion (what moves naturally — suits the source image style)
6. "5 seconds", "no text", "no watermark"

⚠️ DO NOT write the image style name in the prompt body — style is locked via STYLE LOCK above.
⚠️ DO NOT truncate or shorten the character description — include it fully.

Rules:
- ${charNote}
- Animate the existing image — do NOT create a new scene from scratch
- Every scene must be DISTINCT and SPECIFIC to its dialogue
- EN prompt: max 650 characters — if over, cut style decoratives first, NEVER cut character or action
- KO prompt: max 200 characters
...
PROMPT STRUCTURE (comma-separated, natural English, in this exact order):
1. Character & action: ${charNote ? 'MUST match: ' + charNote + ' — ' : ''}what the character is doing RIGHT NOW (specific, not generic)
2. Image content: brief description of EXISTING_IMAGE subject + setting (must match exactly)
3. Motion: what moves and how — subtle, natural, fits the source image style
4. Camera movement — use a DIFFERENT type from adjacent scenes: gentle pan, slow zoom, static hold, tilt — match source style
5. Atmosphere motion (ambient light, shadows, depth — stays within source image visual style)
...
- EN prompt: max 650 characters — if over, cut decorative style details first, NEVER cut character or action
- KO prompt: max 200 characters
- Each scene MUST use a distinct camera movement from the 2 previous scenes
