...
[SCENE]
NUMBER: ${idx}
SCRIPT_CHUNK: ${chunk.slice(0, 1000)}${hintLine}${presenterLine}
TITLE: (장면 제목)
...
[/SCENE]`;
  ...
  if (_seriesName && _episode > 1) {
    ...
    if (_co) {
      const _coParts = [];
      if (_co.last_scene_prompt)      _coParts.push(`🎬 이전 화 마지막 장면 PROMPT: ${_co.last_scene_prompt.slice(0, 300)}`);
      if (_co.last_location)          _coParts.push(`📍 이전 화 마지막 장소: ${_co.last_location}`);
      if (_co.open_threads?.length)   _coParts.push(`🔗 이어지는 스토리 실마리: ${_co.open_threads.join(' / ')}`);
...
- JSON만 출력, 다른 설명 없음`;
    ...
    if (_seriesName) {
      try {
        ...
        console.log(`[Scenes] series_continuity 저장 완료: ${_seriesName} ${_episode}화`);
      } catch (e) {
